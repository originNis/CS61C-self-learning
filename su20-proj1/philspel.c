...
void readDictionary(char *dictName) {
  ...
  const int wordSize = 60;
  char *word = (char *)malloc(sizeof(char) * wordSize);
  int endOfWord = 0;
  char c = fgetc(fp);
  while(c != EOF || endOfWord != 0) {
    if((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
      word[endOfWord] = c;
      endOfWord++;
    } else if(endOfWord != 0) {
...
void processInput() {
  ...
  while(c != EOF || endOfWord != 0) {
    if((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
      ...
      //Try to match primitive word.
      word[endOfWord] = '\0';
      char *temp = (char *)malloc(sizeof(char) * wordSize);
      strcpy(temp, word);
      char *matcheWord = findData(dictionary, word);
      if(matcheWord != NULL) {
        printf("%s", temp);
      } else {
        //Try to match all-lowercase word.
        int i;
        ...
        matcheWord = findData(dictionary, word);
        if(matcheWord != NULL) {
          printf("%s", temp);
        } else {
          //Match when the first letter is capital.
          word[0] -= 32;
          matcheWord = findData(dictionary, word);
          if(matcheWord != NULL) {
            printf("%s", temp);
